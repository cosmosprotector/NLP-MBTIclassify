...
for line in (data['type']):
    #print("line:",line)
    token_list = tokenizer2.texts_to_sequences([line])
    #print("tl:",token_list)
    typescode.append(token_list)
print("sequence:", typescode)
print(len(typescode))
del token_list
gc.collect()
...
for line in (data['clean_text']):
    ...
    for i in range(1, len(token_list)):
        ...
        sentence.append(n_gram_sequence)
...
newseqmatrix= []
for subseq in input_sequences:
    # print(len(subseq))
    input_subsequences = np.array(pad_sequences(subseq, maxlen = max_seq_length, padding='pre'))
    newseqmatrix.append(input_subsequences)
    del input_subsequences
    gc.collect()
    # print(len(newseqmatrix))
print()
print('input_sequences')

newseqmatrix = np.array(newseqmatrix)
typescode = np.array(typescode)

xs, labels = newseqmatrix,typescode
ys = tf.keras.utils.to_categorical(labels, dtype='float64')
X=xs.shape[0]
x_val = xs * VAL_SPLIT
y_val = ys * VAL_SPLIT
print(xs.shape)
